...
//----- REPORT
		...
		{
			 $no = 1;
        $pdf = DB::select('select *from produk_masuks');
        $html = view('infoinven.masuk.reportmasuk')->with('pdf',$pdf)->with('no',$no);
...
public function reportkeluar()
		{
			 $no = 1;
        $pdf = DB::select('select *from produk_keluars');
        $html = view('infoinven.keluar.reportkeluar')->with('pdf',$pdf)->with('no',$no);
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadHTML($html->render())->setPaper('a4')->setOrientation('potrait')->setWarnings(false);
        return $pdf->stream();
		}
...
public function reportpinjaman()
		{
			 $no = 1;
        $pdf = DB::select('select *from produk_pinjamen');
        $html = view('infoinven.peminjaman.reportpeminjaman')->with('pdf',$pdf)->with('no',$no);
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadHTML($html->render())->setPaper('a4')->setOrientation('potrait')->setWarnings(false);
        return $pdf->stream();
		}
